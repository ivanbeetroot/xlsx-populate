...
class Sheet {
    ...
    getX14Validations(){
        if (!this.x14_dataValidations){return null}
        const items = []
        this.x14_dataValidations?.children?.forEach((item)=> {
            const obj = {...item.attributes}
            item?.children?.forEach((child)=>{
                const name = child.name.split(':').pop()
                obj[name] = child?.children?.[0]?.children?.[0] || child?.children?.[0]
            })
            items.push(obj)
        })
        return items
    ...
    _init(workbook, idNode, node, relationshipsNode) {
        ...
        const extNode = extLstNode ? xmlq.findChild(extLstNode, "ext") : null;
        this.x14_dataValidations = extNode ? xmlq.findChild(extNode, "x14:dataValidations") : null;
        if (extLstNode) {
            xmlq.removeChild(this._node, extLstNode);
        }
